...
namespace NumberLetterCounts
{
    class Program
    {
        ...
        private static readonly int HUNDREDS = 8;
        private static readonly int BIGGER_THAN_HUNDREDS = 11;
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (i <= 20) sum += SMALLER_THAN_TWENTY[i];
                if (i < 100)
                {
                    sum += SMALLER_THAN_TWENTY[i % 10];
                    sum += TENS[i % 100];
                }
            }
        }

        static int ReadTwoDigit(int i)
        {
            int sum = 0;
            if (i <= 20)
                sum += SMALLER_THAN_TWENTY[i];
